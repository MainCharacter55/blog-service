...
@section('content')
    <section class="mx-auto w-full max-w-4xl space-y-6">
        <div class="rounded-[1.75rem] border border-white/10 bg-white/5 p-6 backdrop-blur">
            ...
            <h1 class="mt-2 text-3xl font-semibold text-white">Recent Posts</h1>
            <p class="mt-3 text-sm text-slate-300">最新投稿順で投稿を表示しています。</p>
        </div>
                        ...
                        @auth
                            ...
                            <div class="relative" data-reaction-menu-wrapper>
                                <button
                                    ...
                                    aria-expanded="false"
                                    aria-controls="recent-post-reaction-menu-{{ $post->id }}"
                                >
                                    Others{{ $otherPostReactionsCount > 0 ? ' +' . $otherPostReactionsCount : '' }} ▾
                                </button>

                                <div id="recent-post-reaction-menu-{{ $post->id }}" class="absolute left-0 z-20 mt-2 hidden w-64 overflow-hidden rounded-2xl border border-white/10 bg-slate-950 shadow-2xl shadow-black/40" data-reaction-menu>
        ...
        @if ($posts->hasPages())
            <div class="rounded-2xl border border-white/10 bg-slate-900/70 px-4 py-3">
                <nav class="flex flex-wrap items-center justify-center gap-2" aria-label="Recent posts pagination">
